...
# in sync: hacs.json, manifest.json, and the lower bound in tests/requirements.in.
...
bump-min-ha:
	...
	sed -i.bak 's/"homeassistant": "[^"]*"/"homeassistant": "$(VERSION)"/' hacs.json \
		&& rm hacs.json.bak
	sed -i.bak 's/"homeassistant": "[^"]*"/"homeassistant": "$(VERSION)"/' custom_components/duux/manifest.json \
		&& rm custom_components/duux/manifest.json.bak
	...
	@echo "Commit: hacs.json, manifest.json, tests/requirements.in, tests/requirements.txt"
